...
if (isset($_POST['update_holiday'])) {
    ...
    if ($id <= 0 || $holidayDate === '' || $description === '') {
        $error = 'Valid holiday id, date and description are required for update.';
    } else {
        ...
        $error = 'Could not update holiday (date may already exist).';
        $editId = $id;
    }
}
